import {ref} from 'vue'
import {delWarehouseLocation} from "@/api/basic/warehouseLocation.js";
import {delWarehouseContainer} from "@/api/basic/warehouseContainer.js";
...
export default function useDelete(t, proxy) {
    async function deleteHandler(row) {
        ...
        const deleteAndRefresh = async () => {
            await delWarehouseContainer({id:row.id})
            brandIds.value = '';
            idList.value = [];
