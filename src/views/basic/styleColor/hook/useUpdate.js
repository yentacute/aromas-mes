...
import {addStyleColor, updateStyleColor} from "@/api/basic/styleColor.js";
export default function useUpdate() {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            try {
                ...
                if (updateFormData.id) { //编辑
                    await  updateStyleColor(updateFormData)
                } else {//添加
                    await  addStyleColor(updateFormData)
                }
                closeHandle()
                ElMessage({
                    type: 'success',
    ...
    const openHandle= (row, id) => {
        title.value = row && id ? '修改成品色号' :'新增成品色号'
