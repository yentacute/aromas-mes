<template>
  <div class="app-container">
    ...
    <ctn-table
        ...
        @queryTable="getlistCutPartInventory(formData)" v-model:showSearch="showSearch"
        @handleCurrentChange="getlistCutPartInventory(formData)" :total="totalItems" showMultipleColumn v-model:pageSize="formData.pageSize"
...
import { listCutPartInventory } from "@/api/cutpart/cutpart.js";
...
const getlistCutPartInventory = async(params) => {
  try {
    loading.value = true; 
    const response = await listCutPartInventory(params);
...
const handleExport = () => {
  proxy.download(
    "bok/cutPartWarehouseInventoryReport/export",
    formData,
    `cutpart_inventory_report_${new Date().getTime()}.xlsx`
  );
}
...
const handleSearch = () => {
  ...
  getlistCutPartInventory(formData);
}
onMounted(() => {
  getlistCutPartInventory(formData);
})
